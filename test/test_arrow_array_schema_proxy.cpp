    ...
    TEST_CASE("resize_buffer")
    {
        SUBCASE("on sparrow c structure")
        {
            SUBCASE("smaller")
            {
                ...
                CHECK_EQ(buffers[1].size(), 10);
                proxy.set_length(5);
                proxy.resize_buffer(1, 5, 0);
                auto updated_buffers = proxy.buffers();
                REQUIRE_EQ(updated_buffers.size(), 2);
                CHECK_EQ(updated_buffers[0].size(), 1);
                CHECK_EQ(updated_buffers[1].size(), 5);
            }
            ...
            SUBCASE("bigger")
            {
                ...
                CHECK_EQ(buffers[1].size(), 10);
                proxy.set_length(13);
                proxy.resize_buffer(1, 13, 1);
                auto updated_buffers = proxy.buffers();
                REQUIRE_EQ(updated_buffers.size(), 2);
                CHECK_EQ(updated_buffers[0].size(), 2);
                CHECK_EQ(updated_buffers[1].size(), 13);
                CHECK_EQ(updated_buffers[1][9], 9);
                CHECK_EQ(updated_buffers[1][10], 1);
                CHECK_EQ(updated_buffers[1][11], 1);
                CHECK_EQ(updated_buffers[1][12], 1);
            }
        }
        ...
        SUBCASE("on external c structure")
        {
            auto [schema, array] = make_external_arrow_schema_and_array();
            sparrow::arrow_proxy proxy(std::move(array), std::move(schema));
            CHECK_THROWS_AS(proxy.resize_buffer(1, 5, 1), std::runtime_error);
        }
    }
