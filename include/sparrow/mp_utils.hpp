...
namespace sparrow::mpl
{
    ...
    /// Matches any type that is a contiguous range of characters or convertible to string_view.
    template <class T>
    concept string_like = std::is_convertible_v<T, std::string_view>
                          || (std::ranges::contiguous_range<T>
                              && std::same_as<std::ranges::range_value_t<T>, char>);

    /// Matches any type that is a string-like type or a null pointer.
    template <class T>
    concept string_like_or_null = mpl::string_like<T> || std::is_same_v<T, std::nullptr_t>
