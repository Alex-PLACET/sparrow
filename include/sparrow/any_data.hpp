...
namespace sparrow
{
    ...
    public:
        ...
        explicit any_data(std::nullopt_t) noexcept;
        ...
        template <class T>
        explicit any_data(T&& data)
            requires(!mpl::smart_ptr_and_derived<T> && !mpl::string_like<T>);
    ...
    public:
        ...
        explicit any_data_container() noexcept = default;

        explicit any_data_container(std::nullptr_t) noexcept;

        explicit any_data_container(std::nullopt_t) noexcept;
    ...
    inline any_data::any_data(std::nullptr_t) noexcept
    {
    }

    inline any_data::any_data(std::nullopt_t) noexcept
    {
    }

    template <typename T>
    any_data::any_data(T* data)
        : m_raw_ptr(const_cast<std::remove_const_t<T>*>(data))
    {
    }
    ...
    any_data::any_data(U&& data)
        ...
        , m_raw_ptr(std::any_cast<value_ptr_from<U>&>(m_owner).get())
    {
    }
    ...
    any_data::any_data(T data)
        : m_owner(std::move(data))
        , m_raw_ptr(const_cast<T::pointer>(std::any_cast<T&>(m_owner).data()))
    {
    }
    ...
    inline bool any_data::owns_data() const noexcept
    {
        return m_owner.has_value();
    }

    inline std::type_index any_data::type_id() const noexcept
    {
        return m_owner.type();
    }

    inline any_data_container::any_data_container(std::nullptr_t) noexcept
    {
    }

    inline any_data_container::any_data_container(std::nullopt_t) noexcept
    {
    }
    ...
    inline any_data_container::any_data_container(any_data_container&& other) noexcept
    ...
    inline any_data_container& any_data_container::operator=(any_data_container&& other) noexcept
    {
        if (this != &other)
        {
            m_owner = std::move(other.m_owner);
    ...
    inline std::vector<void*>& any_data_container::get_pointers_vec() noexcept
    {
        return m_pointers_vec;
    }

    inline std::span<const void* const> any_data_container::get_pointers_vec() const noexcept
    {
        return m_pointers_vec;
    }
    ...
    T** any_data_container::get() noexcept
    {
        ...
        {
            return const_cast<const T**>(reinterpret_cast<std::remove_const_t<T>**>(m_raw_pointers));
        }
        else
        {
    ...
    inline bool any_data_container::owns_data() const noexcept
    {
        return m_owner.has_value();
    }

    inline std::type_index any_data_container::type_id() const noexcept
    {
        return m_owner.type();
    }
}
